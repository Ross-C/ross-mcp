    ...
    async def _handle_message(self, raw: str) -> Response:
        ...
        try:
            ...
            match cmd.type:
                ...
                case CommandType.DOWNLOAD_ATTACHMENT:
                    p = DownloadAttachmentPayload(**cmd.payload)
                    result = await self.mail.download_attachment(message_id=p.message_id, attachment_index=p.attachment_index)
                # --- Outlook Calendar ---
                ...
                case CommandType.MP_LIST_ACTIVITIES:
                    p = MPListActivitiesPayload(**cmd.payload)
                    result = await self.mp_portal.list_activities(customer_id=p.customer_id, project_id=p.project_id)
